import type { StaleGuardData, StaleGuardEntry } from './types'
...
export function serializeHashFile(data: StaleGuardData): string {
...
export function parseHashFile(content: string): StaleGuardData {
...
function parseSection(section: Record<string, string> | undefined): StaleGuardEntry | undefined {
  if (!section)
    return undefined
...
function parseSectionList(section: Record<string, string> | undefined): StaleGuardEntry[] {
  if (!section)
    return []
  return Object.entries(section).map(([file, hash]) => ({ file, hash }))
}

export async function readHashFile(path: string): Promise<StaleGuardData> {
  const content = await readFile(path, 'utf-8')
  return parseHashFile(content)
}

export async function writeHashFile(path: string, data: StaleGuardData): Promise<void> {
